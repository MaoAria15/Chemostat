...
#OTU table
otu_mat <- read.delim('Data/16S/otu_result_SS99_SM98.tsv', sep = '\t', stringsAsFactors = FALSE, check.names = FALSE, row.names = 1)

#04/11-2021 taxonomy from Josue
tax_mat <- read.delim('Data/16S/sintax_taxa.txt', sep = '\t', stringsAsFactors = FALSE, check.names = FALSE, row.names = 1)
...
map_mat <- read.delim('Data/16S/mapping_human_mice_chemostat_bacteriome_08042024.txt', sep = '\t', stringsAsFactors = FALSE, check.names = FALSE, row.names = 1)

# map_mat <- dplyr::mutate_at(map_mat,1:9,funs(factor))
# map_mat <- dplyr::mutate_if(map_mat, is.character, as.numeric)
TREE = read_tree("Data/16S/zOTU.tree")
