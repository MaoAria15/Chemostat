...
ps = rarefy_even_depth(PSV.no.realm, rngseed=1, sample.size=2000, replace=F)
...
write_xlsx(sheets, "Virome/stat_result/virome_alpha_Mice.xlsx")
...
p_mice <- ggplot(tab, aes(x= Groups, y= Shannon)) +
  ...
  scale_y_continuous(limits = c(2, 4.1))+
  scale_fill_manual(values = cols) +
  labs(x="", y="Shannon diversity index", title="¦Á-diversity Mouse") +
...
write_xlsx(sheets, "Virome/stat_result/virome_alpha_human.xlsx")
...
p_human <- ggplot(tab, aes(x= Groups, y= Shannon)) +
  ...
  scale_fill_manual(values = cols) +
  labs(x="", y="Shannon diversity index", title="¦Á-diversity Human") +
  # stat_pvalue_manual(stat,label = "p.adj.signif", tip.length = 0, size = 6, y.position = c(NA,NA,NA))+
  theme_classic() +
  mytheme_alpha
